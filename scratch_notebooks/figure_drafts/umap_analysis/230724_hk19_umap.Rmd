...
```{r}
escape_df <- read.csv("~/Downloads/230716_escape_df_full.csv", row.names = 1)
```
...
escape_df %>%
    filter(mutant != "*") -> escape_df

# drop duplicate pediatric sample 2463
escape_df %>%
    filter(serum != "2463") -> escape_df

# filter out ferrets for now
escape_df %>%
    filter(age_cohort != "ferret") -> escape_df
...
# helper function to create serum by measurement (ic90 or beta) matrices, aggregate
# measurement by site
CreateFeatureMatSite <- function(df, measure) {
    df$measure <- df[, measure]
    df %>%
        group_by(site, serum) %>%
        summarize(measure = sum(measure)) -> df
    df[, c("site", "measure", "serum")] %>%
        pivot_wider(names_from = serum, values_from = measure) -> df
    mat <- as.matrix(x = df[, -1])
    rownames(x = mat) <- df$site
    mat <- mat[complete.cases(mat), ]
    return(t(x = mat))
}

ic90_aa <- CreateFeatureMat(escape_df, "ic90_mean")
ic90_site <- CreateFeatureMatSite(escape_df, "sitewise_ic90_sum")
```
...
NormalizeSera <- function(mat) {
  mat <- t(apply(X = mat, MARGIN = 1, FUN = function(x) x/max(abs(x))))
  return(mat)
}
ic90_aa_norm <- NormalizeSera(ic90_aa)
ic90_site_norm <- NormalizeSera(ic90_site)
```
...
```{r pca}
aa_pca <- prcomp(ic90_aa_norm)
site_pca <- prcomp(ic90_site_norm)
```
...
serum_to_serum <- sapply(unique(escape_df$serum), FUN = function(x) escape_df[escape_df$serum == x, "serum"][1])
...
RunUMAP <- function(pca_res, dims, serum_to_age) {
  ...
  umap_pca_df <- data.frame(
    ...
    serum= unname(serum_to_serum[rownames(umap_pca_res$layout)])
  )
  return(umap_pca_df)
}
...
aa_umap <- RunUMAP(aa_pca, 1:3, serum_to_age)
p1 <- PlotUMAP(aa_umap) + ggtitle("mutation-level")
site_umap <- RunUMAP(site_pca, 1:5, serum_to_age)
p2 <- PlotUMAP(site_umap) + ggtitle("site-level")
...
TopFeatures(site_pca, dims = 1:10)
```

```{r}
TopFeatures(aa_pca, dims = 1:10)
```
