...
```{r}
escape_df <- read.csv("~/Downloads/230721_perth09_df.csv", row.names = 1)
```
...
aa <- CreateFeatureMat(escape_df, "escape")
site <- CreateFeatureMatSite(escape_df, "escape")
```
...
aa_norm <- NormalizeSera(aa)
site_norm <- NormalizeSera(site)
```
...
```{r pca}
aa_pca <- prcomp(aa_norm)
site_pca <- prcomp(site_norm)
```
...
serum_to_age <- sapply(unique(escape_df$serum), FUN = function(x) escape_df[escape_df$serum == x, "sample_type"][1])
...
aa_umap <- RunUMAP(aa_pca, 1:4, serum_to_age)
p1 <- PlotUMAP(aa_umap) + ggtitle("mutation-level")
site_umap <- RunUMAP(site_pca, 1:4, serum_to_age)
