...
barcode_runs_csv = snakemake.config["barcode_runs"]
...
if not all(bclen == len(bc) for bc in valid_barcodes):
    raise ValueError(f"not all barcodes of same length")
...
counts_valid = (
    counts_valid
    .append(pd.DataFrame({"barcode": missing_valid_barcodes, "count": 0}))
...
fates = (
    fates
    .query("fate not in ['valid barcode', 'invalid barcode']")
    .append(pd.DataFrame({
