...
readstats, aligned, filtered = targets.parse_alignment(samfile,
                                                       to_csv=True,
...
(pd.DataFrame.from_records(list(aligned.items()),
                           columns=['target', 'csv_file'],
                           )
 .to_csv(os.path.join(snakemake.output.outdir, 'aligned.csv'), index=False)
 )
(pd.DataFrame.from_records(list(filtered.items()),
                           columns=['target', 'csv_file'],
                           )
